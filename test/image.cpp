...
/*!
* \brief First constructor: Creating an image from its dimension and number of pixels
*
*/
...
/*!
* \brief Second constructor: Creating an image from its path
* Fixe l'instant à une valeur précise.
* \param image_path permet de savoir le chemin d'une image
*/
image::image(string image_path){
	/*!
	* We assume that the images we will process are grayscale
	* We use the OpenCV's methods to initialize all the attributes
  *
	*/
...
// Third constructor: initializing the attributes from an instance of type Mat
image::image(Mat M){
	height = M.rows;
	width = M.cols;
	nb_of_pixel = height*width;
...
// Now we are going to define some getters in order to return the values of some attributes
...
// To get the value of the pixel at the position (x,y) of the attributes ‘matrix’
unsigned int image::return_pixel_value(unsigned int x,unsigned int y){
	return matrix.at<uchar>(x,y);
}

// In order to save an image after some modification we define the following method
void image::save_image(string saving_name){
	imwrite(saving_name.append(".png"),matrix); // We save them with .png extension
	cout << "Image saved." << endl;
}

// This method returns the maximum intensity of an image
...
// This method returns the minimum intensity of an image
...
void image::print_pressure_center(unsigned int square_size){
	unsigned int intensity_sum = 0;
	float intensity_mean = 0;
	unsigned int max_density = 0;
	unsigned int x_max_intensity = 0;
	unsigned int y_max_intensity = 0;
	if (square_size < height && square_size < width){
			...
			for (int j =0 ; j < width-(square_size - 1)  ; j++){
				for (int k = 0 ; k < square_size ; k++){
					for (int l = 0 ; l < square_size ; l++){
						intensity_sum += 255 - (matrix.at<uchar>(i+k,j+l));
					}
				}
				intensity_mean = intensity_sum/(pow(square_size,2));
				if (intensity_mean > max_density){
					max_density = intensity_mean;
					x_max_intensity = i;
					y_max_intensity = j;
					intensity_mean = 0;
					intensity_sum = 0;
		...
		this->black_square(Coordinates(x_max_intensity,y_max_intensity),Coordinates(x_max_intensity,y_max_intensity) + square_size);
	}
	else {
		cout << "Error : The size of the square is too big for the picture." << endl;
	}
}

Coordinates image::return_pressure_center(unsigned int square_size){
	if (square_size % 2 == 0){
		square_size++;
	}
	unsigned int intensity_sum = 0;
	...
	if (square_size < height && square_size < width){
		...
		Coordinates rep(x_max_intensity + square_size / 2 ,y_max_intensity + square_size / 2);
		return rep;
	}
	else {
		cout << "Error : The size of the square is too big for the picture." << endl;
...
Coordinates image::ellipse_max_dist_from_bary(){
	vector<Coordinates> contour = this->contour();
	vector<float> distance_from_bary(contour.size(),0);
	Coordinates barycenter = this->barycenter();
	float norm_min = (contour.at(0) - barycenter).norm();
	float norm_max = (contour.at(0) - barycenter).norm();
	int x = (contour.at(0)).x_get();
	int y = (contour.at(0)).y_get();
	for (int i =1; i < contour.size(); i++){
		distance_from_bary.at(i) = (contour.at(i) - barycenter).norm();
		if (distance_from_bary.at(i) < norm_min){
			norm_min = distance_from_bary.at(i);
		}
		if (distance_from_bary.at(i) > norm_max){
			norm_max = distance_from_bary.at(i);
			x = (contour.at(i)).x_get();
			y = (contour.at(i)).y_get();
		}
	}
...
void image::print_ellipse(){
	vector<float> parameters = this->ellipse_parameters_gradient(0.00);
	float a = parameters.at(0);
	float b = parameters.at(1);
	Coordinates barycenter = this->barycenter();
	for (int i = 0; i<height; i++){
		for (int j = 0; j<width; j++){
			if (pow((i - barycenter.x_get()),2)/pow(a,2) + pow((j - barycenter.y_get()),2)/pow(b,2) > 0.95
							&& pow((i - barycenter.x_get()),2)/pow(a,2) + pow((j - barycenter.y_get()),2)/pow(b,2) <1.05){
								this->black_square(Coordinates(i,j) - 1,Coordinates(i,j) + 1);
			}
		}
	}
}
...
void image::bigger_image(){ //in a way to save information, we'll start computing the rotations with bigger images
...
void image::smaller_image(){ //to return an image with the exact same dimension of the original one, we have to implement the inverse method of "bigger_image"
...
int image::argmin_error_x(image image_compare){
	int c = 0;
	float S;
...
int image::argmin_error_xy(image image_compare){
	float S;
	int c_x = 0;
	int c_y = 0;
...
int image::argmin_error_xy_bis(image image_compare){
	float f_bar = 0.;
	float gw_bar = 0.;
	float B = 0.;
	float C = 0.;
...
int image::argmin_error_xy_rot(image image_compare){
	float f_bar = 0.;
	float gw_bar = 0.;
	float B = 0.;
	float C = 0.;
	...
	float min = std::numeric_limits<float>::min();
	Mat matrix_compare = image_compare.matrix_get();
	Mat mat3 = this->matrix_get();

for (int m=1;m<=2;m++){
	float x = (float)m/10;
	cout<< " la vel de m est : " << m << " et x est: " << x << endl;
		this->matrix = mat3.clone();
		this->rotation_interpolation_bicubis(this->barycenter(), PI/4);
...
for (int i=0;i<=2*height-2*reduc_x;i++){
	...
	cout << "La translation suivant x : " << c_x << endl;
	cout << "La translation suivant y : " << c_y << endl;
	return 0;
}
...
void image::detect_warp(image image_compare,Coordinates center,int nb_test_rot, int nb_testx, int nb_testy){
	image depart(this->matrix_get());
	image imtmp = depart;
	float tmp;
	vector<float> rep(3);
	...
	cout << "Rotation : " << rot_rep/PI << " PI" << endl;
	cout << "Translation : " << rep.at(0) << " et " << rep.at(1) << endl;
	cout << "Error : " << rep.at(2) << endl;
	depart.rotation_interpolation_bicubis(center,rot_rep);
	depart.translation(rep.at(0),rep.at(1));
	cout << depart;
	cout << image_compare;
}
...
/*g++ image.cpp -o exe `pkg-config --libs --cflags opencv`*/
